...
export default function Purchases() {
  ...
  const net = total - (parseFloat(form.commission) || 0) - (parseFloat(form.bardana) || 0) - (parseFloat(form.labour) || 0) - (parseFloat(form.munshiyana) || 0) - (parseFloat(form.kiraya) || 0) - (parseFloat(form.others) || 0) + extraExpensesTotal;
  ...
  const openEditForm = (p) => {
    ...
    setShowUnitInput(false); setNewUnit('');
    setExtraExpenses(p.extra_expenses ? JSON.parse(p.extra_expenses) : []); setShowExpensesPanel(false);
    setForm({
      ...
      <Modal show={showForm} onClose={() => { setShowForm(false); setEditingId(null); }} title={editingId ? <>Edit Purchase — <span className="urdu">خریداری میں ترمیم</span></> : <>New Purchase — <span className="urdu">نئی خریداری</span></>} large>
        <form onSubmit={handleSave}>
          ...
          <div className="modal-footer">
            ...
            <button type="submit" className="btn btn-primary">{editingId ? 'Update Purchase' : 'Save Purchase'}</button>
          </div>
        </form>
      </Modal>
    </div>
